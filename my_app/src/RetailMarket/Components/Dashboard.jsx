...
const Card = ({ children }) => (
  <div style={{ background: '#fff', borderRadius: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', padding: '16px' }}>
    {children}
  </div>
);
...
const Dashboard = () => {
  ...
  const revenueData = Object.entries(dailySales).map(([date, total]) => ({
    date,
    current: total
  }));
  ...
  const stackChartData = Object.entries(stackDateMap).map(([date, totalStacks]) => ({
    date,
    totalStacks
  }));
  ...
  return (
    <div style={{ padding: '16px', display: 'grid', gap: '16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
        <Card>
          <CardContent>
            <h4 style={{ fontSize: '14px', fontWeight: 600 }}>Credited Amount</h4>
            <p style={{ fontSize: '20px', fontWeight: 'bold' }}>₹{creditTotal.toFixed(2)}</p>
            <p style={{ fontSize: '12px', color: 'green' }}>Total from Credit payments</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <h4 style={{ fontSize: '14px', fontWeight: 600 }}>Debited Amount</h4>
            <p style={{ fontSize: '20px', fontWeight: 'bold' }}>₹{debitTotal.toFixed(2)}</p>
            <p style={{ fontSize: '12px', color: 'red' }}>Total from Debit payments</p>
          </CardContent>
        </Card>
      </div>
